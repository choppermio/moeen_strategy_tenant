...
<div class="{{ $cardClass }}" data-employee-id="{{ $employee['id'] }}" style="width: 280px; display: inline-block; vertical-align: top;">
    ...
    <div class="card-body">
        ...
        <div class="performance-bar">
            <div class="performance-fill" style="width: {{ $employee['avg_performance'] }}%"></div>
        </div>
    ...
    @if(count($employee['children']) > 0)
        ...
        <div class="children-container">
            @foreach($employee['children'] as $child)
                @include('stats.partials.employee-card', ['employee' => $child, 'level' => $level + 1])
            @endforeach
        </div>
    @endif
</div>
